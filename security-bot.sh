...
echo "== KAIIZXXXY SECURITY INSTALLER (BOT VERSION) =="
echo "App: $APP_DIR"
...
class CustomSecurityCheck
{
    public function handle(Request $request, Closure $next)
    {
        ...
        if ($this->isAccessingAdminPanel($path, $method)) {
            ...
            return $this->deny($request, 'Hanya Super Admin (ID 1) yang boleh akses Admin Panel! - @kaiizxxxy');
        }
        ...
        if ($this->isAccessingAdminAPI($path, $method)) {
            ...
            return $this->deny($request, 'Hanya Super Admin (ID 1) yang boleh akses Admin API! - @kaiizxxxy');
        }
        ...
        if ($this->isAccessingSettings($path, $method)) {
            ...
            return $this->deny($request, 'Hanya Super Admin (ID 1) yang boleh akses Settings! - @kaiizxxxy');
        }
        ...
        if ($server instanceof Server) {
            ...
            if (!$isServerOwner && $this->isServerPowerOperation($path, $method)) {
                ...
                return $this->deny($request, 'Hanya pemilik server yang boleh kontrol power! - @kaiizxxxy');
            }
            ...
            if (!$isServerOwner && $this->isAccessingFileManager($path, $method)) {
                ...
                return $this->deny($request, 'File manager tidak dapat diakses! - @kaiizxxxy');
            }
            ...
            if (!$isServerOwner && $this->isBackupOperation($path, $method)) {
                ...
                return $this->deny($request, 'Hanya pemilik server yang boleh akses backup! - @kaiizxxxy');
            }
        }
...
PHP
    ...
    log "✅ KAIIZXXXY SECURITY installed successfully!"
    echo
    ...
    log ""
    log "💬 Created by KAIIZXXXY - VVIP TOOLS"
